...
Additionally, programming practices used while developing took the [Cerner compatible practices](https://fhir.cerner.com/millennium/dstu2/) into account. Specifically, we made sure that bad web responses and client issues were addressed and redirected to real pages, that operation errors were handled, that required database model fields were appropriate, and that security was robust. Other requirements were taken care of automatically by Django, such as correct HTML verbs, authorization, data filtering, etc. The app was submitted to Cerner for review on
